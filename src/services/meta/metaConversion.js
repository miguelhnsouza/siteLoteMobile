let metaAccessToken =
  "EAALsQWbZCId0BPZC9qfDNtfZBomAZBIZBeZCj98xrrWCWVjOog8fZCqWe5Cnosa3Jq46KR32IwwZAPu3DlGjSN7voAxT6HNqd0a2Jj6vJfCCUnDOJYjO8JKVEayZAuYh8QlrWG9HcLVDziAF4ffjeEVCSS0mQ7ZAhfCV03Q0sw6usHl5talZBQkFwanFOze2Gug08e2aAZDZD";
let pixelId = 2703375596599994;
...
const META_ACCESS_TOKEN =
  "225623354EAALsQWbZCId0BPZC9qfDNtfZBomAZBIZBeZCj98xrrWCWVjOog8fZCqWe5Cnosa3Jq46KR32IwwZAPu3DlGjSN7voAxT6HNqd0a2Jj6vJfCCUnDOJYjO8JKVEayZAuYh8QlrWG9HcLVDziAF4ffjeEVCSS0mQ7ZAhfCV03Q0sw6usHl5talZBQkFwanFOze2Gug08e2aAZDZD4814402";
const PIXEL_ID = "2703375596599994";
...
export async function sendMetaConversion(
  ...
  try {
    ...
    if (!metaAccessToken || !pixelId) {
      throw new Error(
        "Meta credentials não configuradas. Use setMetaCredentials primeiro."
      );
    }

    const response = await fetch(
      `https://graph.facebook.com/v19.0/${pixelId}/events?access_token=${metaAccessToken}`,
      {
        method: "POST",
        headers: {
    ...
    console.log(
      `Evento ${eventName} enviado para Meta Conversion API:`,
      result
    );
    return result;
  } catch (error) {
...
export async function trackLead(source, userData = {}) {
  ...
  const hashedUserData = {
    ...
    ph: userData.telefone
      ? await hashSHA256(userData.telefone.replace(/\D/g, ""))
      : undefined,
    fn: userData.nome
      ? await hashSHA256(userData.nome.split(" ")[0])
      : undefined,
    ln: userData.nome
      ? await hashSHA256(userData.nome.split(" ").slice(1).join(" "))
      : undefined,
    ...
    zp: userData.cep
      ? await hashSHA256(userData.cep.replace(/\D/g, ""))
      : undefined,
